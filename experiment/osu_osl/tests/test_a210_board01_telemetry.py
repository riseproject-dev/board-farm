...
def test_a210_kernel_uname(target: Target):
    ...
    print(f"\n[Phase 2C Board 1] Kernel: {uname_str}")
...
def test_a210_cpuinfo_isa(target: Target):
    ...
    print("\n[Phase 2C Board 1] Verified 8x XuanTie cores")
...
def test_a210_network_mac(target: Target):
    ...
    assert returncode == 0
    mac = stdout[0].strip()
    print(f"\n[Phase 2C Board 1] Verified end0 MAC: {mac}")
    assert mac == "48:da:00:00:01:00"
...
def test_a210_memory(target: Target):
    ...
    print(f"\n[Phase 2C Board 1] Total RAM: {mem_total_gb:.2f} GB")
